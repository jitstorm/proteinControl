...
| 8 | Z- 目标为 0 时触发 S3 | Z 立即停止、归零并正常完成；后续阶段继续 | 待测 |
| 9 | X- 目标为 0 时触发 S1 | X 立即停止、归零并正常完成；后续阶段继续 | 待测 |
| 10 | Y- 目标为 0 时触发 S2 | Y 立即停止、归零并正常完成；后续阶段继续 | 待测 |
| 11 | 已在 S1/S2/S3 零点继续请求负向 | 启动前返回 LIMIT，不产生 STEP | 待测 |
