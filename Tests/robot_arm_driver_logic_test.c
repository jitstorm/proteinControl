                              ...
                              uint32_t maximum, uint32_t acceleration)
{
    ...
    (void)acceleration;
    s_x_steps = steps;
    s_x_running = s_allow_start;
}

/** 模拟 PB11 停止。 */
void stepdma_pb11_stop(void) { s_x_running = 0u; }
/** 返回 PB11 模拟运行态。 */
uint8_t stepdma_pb11_is_running(void) { return s_x_running; }
/** 返回 PB11 模拟剩余步数。 */
uint32_t stepdma_pb11_get_remaining_steps(void) { return s_x_steps; }
/** 返回 PB11 模拟完成步数。 */
                       ...
                       uint32_t target_frequency)
{
    ...
    s_y_direction = direction;
    s_y_steps = steps;
    s_y_running = s_allow_start;
    return 1u;
}

/** 模拟 PB10 停止。 */
void Stepper2_Stop(void) { s_y_running = 0u; }
/** 返回 PB10 模拟运行态。 */
uint8_t Stepper2_IsBusy(void) { return s_y_running; }
/** 返回 PB10 模拟剩余步数。 */
uint32_t Stepper2_GetRemainingSteps(void) { return s_y_steps; }
/** 返回 PB10 模拟完成步数。 */
...
int main(void)
{
    ...
    TEST_CHECK(s_x_steps == 1427u);
    TEST_CHECK((HC595Data[1] & (1u << 4)) != 0u);
    RobotArmDriver_Stop(ROBOT_AXIS_X);

    TEST_CHECK(RobotArmDriver_Start(ROBOT_AXIS_Y, -1, 23u, 1000u) == 1u);
    TEST_CHECK(s_y_steps == 23u);
    TEST_CHECK(s_y_direction == 0u);
